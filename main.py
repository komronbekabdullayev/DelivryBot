...
WELCOME_MESSAGE = f"<b>Hello! Welcome to 'Three Broomsticks' delivery service.</b>" \
             "\n\n<b>Assalomu alaykum! 'Three Broomsticks' yetkazib berish xizmatiga xush kelibsiz</b>" \
...
async def cmd_start(message: Message, state: FSMContext):
    ...
    kb = [[
        types.KeyboardButton(text="🇷🇺 Russkiy"), 
...
async def process_city(message: Message, state: FSMContext):
    ...
    with open("db/data.csv", mode='a', newline='\n') as file:
        fieldnames = ['user_id', 'language', 'city']
        writer = csv.DictWriter(file, fieldnames=fieldnames)
        writer.writerow({'user_id': user, 'language': user_data['lang'], 'city': user_data['city']})
